...
select a.* from daily_sales a
where tot =
(select max(tot) from daily_sales b where sales_month = a.sales_month
)
order by 1,2
;

-- This kind of subquery is unsupported
with 
daily_sales as
(
select date_trunc(month, O_ORDERDATE) as sales_month, O_ORDERDATE, sum(o_totalprice) as tot from orders 
...
select a.* from daily_sales a
where tot =
(select maxtot from maxb b where maxtot = a.tot and sales_month = a.sales_month
)
;
-- Result: Unsupported subquery type cannot be evaluated
